...
This guide covers manually creating the `myapp` namespace, creating a TLS secret (`apsis-tls`), and setting up an ArgoCD project and application using the Web UI and CLI for a public Git repository. The application uses the Git repository `https://github.com/jamaldevsecops/ArgoCD.git`, manifest path `k8s-manifest/myapp/for-nginx-ingress-contoller`, and deploys to the `myapp` namespace. The ArgoCD UI is accessible at `https://argocd.apsissolutions.com`, secured with Kubernetes TLS. A DNS record for `argocd.apsissolutions.com` is already pointed to the NGINX Ingress Controller's load balancer IP.
...
- Public Git repository (`https://github.com/jamaldevsecops/ArgoCD.git`) with Kubernetes manifests in `k8s-manifest/myapp/for-nginx-ingress-contoller`.
...
## 1. Manually Creating the Namespace (If K8S Namespace Resource Not Available in the Repository) 
...
2. **Create a Project**:
   ...
   - Fill in the details:
     ...
     - **Source Repositories**: Add `https://github.com/jamaldevsecops/ArgoCD.git` (or `*` to allow all repositories).
     - **Destinations**:
...
2. **Create Project**:
   ...
   spec:
     description: Project for myapp deployments
     sourceRepos:
     - https://github.com/jamaldevsecops/ArgoCD.git
     destinations:
     - namespace: myapp
...
4. **Verify Project**:
   ...
   myapp-project Project for myapp deployments myapp,https://kubernetes.default.svc https://github.com/jamaldevsecops/ArgoCD.git *
   ```

## 4. Creating a New Application

This section covers creating an application linked to the public repository `https://github.com/jamaldevsecops/ArgoCD.git` with manifests in `k8s-manifest/myapp/for-nginx-ingress-contoller`, deploying to the `myapp` namespace. The repository is public, so no credentials are required.
...
4. **Verify Application**:
   ...
   Repo:              https://github.com/jamaldevsecops/ArgoCD.git
   Target:            HEAD
   Path:              k8s-manifest/myapp/for-nginx-ingress-contoller
...
1. **Verify Resources**:
   ...
   ```
   Check for pods, services, and ingress resources deployed from the manifests in `k8s-manifest/myapp/for-nginx-ingress-contoller`.
